...
public class LoginHelper {
    ...
    public static class LoginBuilder {
        ...
        public LoginBuilder tcpHostName(String nadronTcpHostName) {
            this.tcpHostName = nadronTcpHostName;
            return this;
        }
        ...
        public LoginBuilder nadronUdpHostName(String nadronUdpHostName) {
            this.udpHostName = nadronUdpHostName;
            return this;
        }
        ...
        protected void validateAndSetValues() {
            ...
            if (null == tcpServerAddress && (null == tcpHostName || null == tcpPort)) {
                throw new IllegalArgumentException("tcpServerAddress cannot be null");
            }

            if (null == tcpServerAddress) {
                tcpServerAddress = new InetSocketAddress(tcpHostName, tcpPort);
            }

            if (null == udpServerAddress) {
                if (null != udpHostName && null != udpPort) {
                    udpServerAddress = new InetSocketAddress(udpHostName, udpPort);
                }
            }
        }
    }
     ...
     *
     * @param localUDPAddress <b>optional</b> If passed in, then this address is passed on
    ...
    public MessageBuffer<ByteBuf> getLoginBuffer(InetSocketAddress localUDPAddress) throws Exception {
        ByteBuf loginBuffer;
        ByteBuf credentials = NettyUtils.writeStrings(username, password, connectionKey);
     ...
     * payload.
     *
     * @param reconnectKey The key that was initially sent by server on game room join
     *                     success event will be sent back to server for reconnecting
     * @param udpAddress   If udp connection is required, then the new udpAddress need to
     *                     be sent.
